...
impl LambdaService {
    ...
    fn resolve_action(req: &AwsRequest) -> Option<(&'static str, Option<String>)> {
        ...
        // Concurrency (reserved + provisioned) — any prefix (legacy
        // 2017-10-31 catch-all kept for posterity; new entries handled
        // above).
        if false {
            let res = segs.get(2).map(|s| s.to_string());
            return match (
                ...
                (&Method::PUT, 4, Some("functions"), Some("concurrency")) => {
                    Some(("PutFunctionConcurrency", res))
                }
                (&Method::GET, 4, Some("functions"), Some("concurrency")) => {
                    Some(("GetFunctionConcurrency", res))
                }
                (&Method::DELETE, 4, Some("functions"), Some("concurrency")) => {
                    Some(("DeleteFunctionConcurrency", res))
                }
                _ => None,
            };
        }

        // Event invoke config at 2019-09-25.
        if prefix == "2019-09-25" {
            let res = segs.get(2).map(|s| s.to_string());
            return match (
                &req.method,
                segs.len(),
                ...
                (&Method::POST, 4, Some("functions"), Some("event-invoke-config")) => {
                    Some(("PutFunctionEventInvokeConfig", res))
                }
                (&Method::PUT, 4, Some("functions"), Some("event-invoke-config")) => {
                    Some(("UpdateFunctionEventInvokeConfig", res))
                }
                (&Method::GET, 4, Some("functions"), Some("event-invoke-config")) => {
                    Some(("GetFunctionEventInvokeConfig", res))
                }
                (&Method::DELETE, 4, Some("functions"), Some("event-invoke-config")) => {
                    Some(("DeleteFunctionEventInvokeConfig", res))
                }
                (&Method::GET, 4, Some("functions"), Some("event-invoke-config/list")) => {
                    Some(("ListFunctionEventInvokeConfigs", res))
                }
                _ => None,
            };
        }

        // Recursion config at 2024-08-31.
        if prefix == "2024-08-31" {
            let res = segs.get(2).map(|s| s.to_string());
            return match (
                &req.method,
                segs.len(),
                ...
                (&Method::PUT, 4, Some("functions"), Some("recursion-config")) => {
                    Some(("PutFunctionRecursionConfig", res))
                }
                (&Method::GET, 4, Some("functions"), Some("recursion-config")) => {
                    Some(("GetFunctionRecursionConfig", res))
                }
                _ => None,
            };
        }

        // Capacity providers (newer date prefix used by SDK 1.x).
        if prefix == "2025-11-30" || prefix == "2025-04-01" {
            let res = segs.get(2).map(|s| s.to_string());
            return match (
                &req.method,
                segs.len(),
                ...
                (&Method::POST, 2, Some("capacity-providers"), _) => {
                    Some(("CreateCapacityProvider", None))
                }
                (&Method::GET, 2, Some("capacity-providers"), _) => {
                    Some(("ListCapacityProviders", None))
                }
                (&Method::GET, 3, Some("capacity-providers"), _) => {
                    Some(("GetCapacityProvider", res))
                }
                (&Method::PUT, 3, Some("capacity-providers"), _) => {
                    Some(("UpdateCapacityProvider", res))
                }
                (&Method::DELETE, 3, Some("capacity-providers"), _) => {
                    Some(("DeleteCapacityProvider", res))
                }
                (&Method::GET, 4, Some("capacity-providers"), Some("function-versions")) => {
                    Some(("ListFunctionVersionsByCapacityProvider", res))
                }
                _ => None,
            };
        ...
        // Durable executions: 2024-09-01 + 2025-12-01 (SDK current).
        if prefix == "2024-09-01" || prefix == "2025-12-01" {
            let res = segs.get(2).map(|s| s.to_string());
            return match (
                &req.method,
                segs.len(),
                ...
                (&Method::GET, 3, Some("durable-executions"), _, _) => {
                    Some(("GetDurableExecution", res))
                }
                (&Method::GET, 4, Some("durable-executions"), Some("history"), _) => {
                    Some(("GetDurableExecutionHistory", res))
                }
                (&Method::GET, 4, Some("durable-executions"), Some("state"), _) => {
                    Some(("GetDurableExecutionState", res))
                }
                (&Method::POST, 4, Some("durable-executions"), Some("checkpoint"), _) => {
                    Some(("CheckpointDurableExecution", res))
                }
                (&Method::POST, 4, Some("durable-executions"), Some("stop"), _) => {
                    Some(("StopDurableExecution", res))
                }
                (
                    &Method::POST,
                    5,
                    Some("durable-executions"),
                    Some("callback"),
                    Some("success"),
                ) => Some(("SendDurableExecutionCallbackSuccess", res)),
                (
        ...
        // /2019-09-25/functions/{name}/policy?Qualifier=...&Action=...
        // (Function URL configs are 2021-10-31)

        // /2020-04-22/code-signing-configs
        if prefix == "2020-04-22" {
            ...
            return match (
                ...
                (&Method::POST, 2, Some("code-signing-configs"), _) => {
                    Some(("CreateCodeSigningConfig", None))
                }
                (&Method::GET, 2, Some("code-signing-configs"), _) => {
                    Some(("ListCodeSigningConfigs", None))
                }
                (&Method::GET, 3, Some("code-signing-configs"), _) => {
                    Some(("GetCodeSigningConfig", res))
                }
                (&Method::PUT, 3, Some("code-signing-configs"), _) => {
                    Some(("UpdateCodeSigningConfig", res))
                }
                (&Method::DELETE, 3, Some("code-signing-configs"), _) => {
                    Some(("DeleteCodeSigningConfig", res))
                }
                (&Method::GET, 4, Some("code-signing-configs"), Some("functions")) => {
                    Some(("ListFunctionsByCodeSigningConfig", res))
                }
                _ => None,
            };
        ...
        if prefix == "2021-10-31" {
            ...
            return match (
                ...
                (&Method::POST, 4, Some("functions"), Some("url")) => {
                    Some(("CreateFunctionUrlConfig", res))
                }
                (&Method::GET, 4, Some("functions"), Some("url")) => {
                    Some(("GetFunctionUrlConfig", res))
                }
                (&Method::PUT, 4, Some("functions"), Some("url")) => {
                    Some(("UpdateFunctionUrlConfig", res))
                }
                (&Method::DELETE, 4, Some("functions"), Some("url")) => {
                    Some(("DeleteFunctionUrlConfig", res))
                }
                (&Method::GET, 3, Some("functions"), _)
                    if segs.get(2).map(|s| s.as_str()) == Some("urls") =>
                {
                    Some(("ListFunctionUrlConfigs", None))
                }
                _ => None,
            };
        }
