...
fakecloud implements 29 AWS services with 2,208 total operations, all at 100% Smithy conformance. Per-service feature matrices and gotchas live on individual service pages — use the sidebar to navigate.
...
| Route 53                  |  13 | **Hosted zones + resource record sets** — CRUD with default SOA/NS records seeded on create, deterministic `INSYNC` change tracking, comment + features updates, hosted zone limits, list-by-name, `TestDNSAnswer` synthesis. REST-XML protocol with HTTP method + URI routing under `/2013-04-01/`. Health checks, traffic policies, DNSSEC, query logging, CIDR collections, VPC associations, reusable delegation sets land in subsequent batches |
