...
fakecloud implements 29 AWS services with 2,232 total operations, all at 100% Smithy conformance. Per-service feature matrices and gotchas live on individual service pages — use the sidebar to navigate.
...
| Route 53                  |  37 | **Hosted zones + resource record sets + health checks + traffic policies + instances** — CRUD with default SOA/NS records seeded on create, deterministic `INSYNC` change tracking, comment + features updates, hosted zone limits, list-by-name, `TestDNSAnswer` synthesis. **Health checks**: full lifecycle with `HealthCheckVersion` optimistic concurrency, `ResetElements` semantics, `HealthCheckInUse` rejection when a record set still references the check, status + last-failure observations, checker IP ranges. **Traffic policies + instances**: versioned policies (CreateTrafficPolicyVersion increments), `TrafficPolicyAlreadyExists` on duplicate name, `TrafficPolicyInUse` rejection on delete while an instance still references the version, `RecordType` inferred from the JSON document, `TrafficPolicyInstanceAlreadyExists` on duplicate `(zone, name, type)`, list-by-hosted-zone + list-by-policy + count. REST-XML protocol with HTTP method + URI routing under `/2013-04-01/`. DNSSEC, query logging, CIDR collections, VPC associations, reusable delegation sets land in subsequent batches |
