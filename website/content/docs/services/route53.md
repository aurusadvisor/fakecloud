...
- `POST /_fakecloud/route53/health-checks/{id}/status` — flip a stored health check's reported status. Body `{"status": "Failure", "reason": "Endpoint timed out"}` makes `GetHealthCheckStatus` return `Failure: Endpoint timed out` and `GetHealthCheckLastFailureReason` surface the same reason. Body `{"status": "Success"}` clears the reported failure (the historical reason is preserved). Returns `204 No Content` on success and `404 Not Found` for an unknown id. Available in every fakecloud SDK as `route53.setHealthCheckStatus(id, ...)` (or the language-idiomatic equivalent).
