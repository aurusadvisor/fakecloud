...
| [CloudWatch Logs](@/docs/services/logs.md) | 113 | JSON 1.1 | Full | Full | `StartLiveTail` returns the buffered tail of matching log groups (single envelope rather than a streamed eventstream). `GetLogObject` resolves pointers minted by other operations and returns the underlying event. `GetLogFields` aggregates the JSON keys observed across the source's events. Log event export to S3 and Firehose is real. Metric filters extract metrics from ingested logs. |
