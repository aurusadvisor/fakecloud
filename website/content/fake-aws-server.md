...
- **Speaks the AWS wire protocol** at 100% conformance per implemented service. 27 services, 2,048 operations, validated against AWS's own Smithy models on every commit (59,000+ generated test variants).
...
S3, SQS, SNS, DynamoDB, Lambda, IAM, STS, KMS, Secrets Manager, SSM, CloudWatch Logs, CloudFormation, EventBridge, EventBridge Scheduler, SES (v2 + v1 inbound), Cognito User Pools, Kinesis, RDS, ElastiCache, Step Functions, API Gateway v1 (REST), API Gateway v2 (HTTP), Bedrock, Bedrock Runtime.
